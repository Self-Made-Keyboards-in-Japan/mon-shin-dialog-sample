...
export function linkNameToQuestionaire() {
	const nameToQuestionaire = new Map();
...
export function linkProblemAndTextbox(e) {
	switch (e.target.id) {
		...
		case "otherBuildProblem":
			...
			document.getElementById(e.target.dataset.relationTextarea).disabled =
				!e.target.checked;
			break;
		default:
			break;
...
export function generatePostText(nameToQuestionaire) {
	...
	for (const key of form_data.keys()) {
		if (nameToQuestionaire.has(key)) {
			userText =
				"__**" + nameToQuestionaire.get(key) + "**__\n" + form_data.get(key);
...
export function checkDiscordLimit(postsText) {
	...
	if (character + lineBreak > 2001) {
		...
		if (toggleBtn.checked) {
			alertTextDiscord.innerText =
				'Limitation of Discord is "character + line ≦ 2001".';
		} else if (!toggleBtn.checked) {
			alertTextDiscord.innerText =
				"Discord の投稿欄の制限は「文字数＋行数 ≦ 2001」です。";
		}
	...
	if (toggleBtn.checked) {
		charactersAndLines.innerText =
			"character + lines = " + (character + lineBreak);
	} else if (!toggleBtn.checked) {
...
export function splitKeyboardLinked(e) {
	const replaceConnectSideYes = document.getElementById(
		"replaceConnectSideYes",
	);
...
export function updateContent(
	e,
	enLocalesTranslationJson,
	jaLocalesTranslationJson,
) {
	if (e.target.checked) {
